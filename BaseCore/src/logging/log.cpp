...
void LogBufferWriter::attach(LogShmHeader* header, LogEntry* buffer, std::size_t capacity, uint8_t* buffer_var,
                             std::size_t buffer_var_size) noexcept {
    header_ = header;
    buffer_ = buffer;
...
bool LogBufferWriter::try_push_variable(uint64_t ts_tsc, uint8_t type_id, uint8_t level, uint8_t msg_id,
...
bool LogBufferWriter::try_push_variable_str(uint64_t ts_tsc, uint8_t level, uint8_t module_id, const char* str,
...
bool ShmLogger::init(const ShmLogConfig& config) {
    ...
    const std::size_t layout_size =
        sizeof(LogShmHeader) + config.buffer_capacity * sizeof(LogEntry) + ShmLogger::kBufferVarSize;
    ...
    uint8_t* var_buf = (hdr->version == 3 && layout_size >= ShmLogger::kLayoutSize) ? buf_var : nullptr;
...
LogBufferWriter* ShmLogger::writer() noexcept { return layout_ptr_ ? &writer_impl_ : nullptr; }

const LogBufferWriter* ShmLogger::writer() const noexcept { return layout_ptr_ ? &writer_impl_ : nullptr; }

LogShmHeader* ShmLogger::header() noexcept { return static_cast<LogShmHeader*>(layout_ptr_); }

const LogShmHeader* ShmLogger::header() const noexcept { return static_cast<const LogShmHeader*>(layout_ptr_); }
...
static void format_timestamp_ns(uint64_t ns, char* buf, std::size_t buf_size) {
    ...
    struct tm tm;
    gmtime_r(&sec, &tm);
    std::snprintf(buf, buf_size, "%04d-%02d-%02d %02d:%02d:%02d.%09u", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
...
void decode_log_entry(const LogEntry& e, const LogShmHeader* header, char* buf, std::size_t buf_size,
    ...
    const char* mod = (e.module_id != 0 && module_mapper) ? module_mapper(e.module_id) : "unknown";
    int n = 0;

    switch (static_cast<LogTypeId>(e.type_id)) {
        case LogTypeId::MsgFormat: {
            char msg_buf[256];
            if (base_core_log::LogFormatRegistry::decode(e.msg_id, e.payload, e.payload_size, msg_buf,
                                                         sizeof(msg_buf))) {
                n = std::snprintf(buf, buf_size, "[%s][%s][%s] %s\n", time_buf,
                                  to_string(static_cast<LogLevel>(e.level)), mod, msg_buf);
            } else {
                n = std::snprintf(buf, buf_size, "[%s][%s][%s] [decode error format_id=%u]\n", time_buf,
                                  to_string(static_cast<LogLevel>(e.level)), mod, e.msg_id);
            }
            break;
        }
        case LogTypeId::MsgString: {
            ...
            str_buf[copy_len] = '\0';
            n = std::snprintf(buf, buf_size, "[%s][%s][%s] %s\n", time_buf, to_string(static_cast<LogLevel>(e.level)),
                              mod, str_buf);
            break;
        }
        default:
            n = std::snprintf(buf, buf_size, "[%s][%s][%s] unknown type_id=%u\n", time_buf,
                              to_string(static_cast<LogLevel>(e.level)), mod, e.type_id);
    }
    (void)n;
}

void decode_log_entry_var(const LogVarEntryHeader& hdr, const uint8_t* payload, const LogShmHeader* header, char* buf,
                          std::size_t buf_size, ModuleNameMapper module_mapper) {
    ...
    const char* mod = (hdr.module_id != 0 && module_mapper) ? module_mapper(hdr.module_id) : "unknown";
    int n = 0;

    switch (static_cast<LogTypeId>(hdr.type_id)) {
        case LogTypeId::MsgFormat: {
            char msg_buf[256];
            if (base_core_log::LogFormatRegistry::decode(hdr.msg_id, payload, hdr.payload_size, msg_buf,
                                                         sizeof(msg_buf))) {
                n = std::snprintf(buf, buf_size, "[%s][%s][%s] %s\n", time_buf,
                                  to_string(static_cast<LogLevel>(hdr.level)), mod, msg_buf);
            } else {
                n = std::snprintf(buf, buf_size, "[%s][%s][%s] [decode error format_id=%u]\n", time_buf,
                                  to_string(static_cast<LogLevel>(hdr.level)), mod, hdr.msg_id);
            }
            break;
        }
        case LogTypeId::MsgString: {
            char str_buf[kLogMaxVarPayload + 1];
            const std::size_t copy_len = (static_cast<std::size_t>(hdr.payload_size) <= kLogMaxVarPayload)
                                             ? static_cast<std::size_t>(hdr.payload_size)
                                             : kLogMaxVarPayload;
            std::memcpy(str_buf, payload, copy_len);
            str_buf[copy_len] = '\0';
            n = std::snprintf(buf, buf_size, "[%s][%s][%s] %s\n", time_buf, to_string(static_cast<LogLevel>(hdr.level)),
                              mod, str_buf);
            break;
        }
        default:
            n = std::snprintf(buf, buf_size, "[%s][%s][%s] unknown type_id=%u\n", time_buf,
                              to_string(static_cast<LogLevel>(hdr.level)), mod, hdr.type_id);
    }
    (void)n;
}
...
LogReader::LogReader(std::string shm_name, std::string output_path, ModuleNameMapper module_mapper)
    : shm_name_(std::move(shm_name)), output_path_(std::move(output_path)), module_mapper_(module_mapper) {}

int LogReader::run() {
    shm::ShmWriter writer;
    void* ptr = writer.open(shm_name_, ShmLogger::kLayoutSize);
    if (!ptr) {
        constexpr std::size_t kV2LayoutSize = sizeof(LogShmHeader) + ShmLogger::kBufferCapacity * sizeof(LogEntry);
        ptr = writer.open(shm_name_, kV2LayoutSize);
    }
    if (!ptr) {
        shm::log_error("logReader: failed to open shm name=" + shm_name_);
        std::cerr << "logReader: failed to open shm '" << shm_name_ << "'\n";
        return 1;
    }

    const auto* header = static_cast<const LogShmHeader*>(ptr);
    if (header->magic != 0x4C4F4753) {
        shm::log_error("logReader: invalid shm magic name=" + shm_name_);
        std::cerr << "logReader: invalid shm magic\n";
        writer.close();
        return 1;
    }
    if (header->version != 2 && header->version != 3) {
        ...
        std::cerr << "logReader: unsupported shm version " << header->version << "\n";
        writer.close();
        return 1;
    }

    const auto* base = static_cast<const char*>(ptr) + sizeof(LogShmHeader);
    const auto* buffer = reinterpret_cast<const LogEntry*>(base);
    ...
    if (!out) {
        ...
        std::cerr << "logReader: failed to open output '" << output_path_ << "'\n";
        writer.close();
        return 1;
    }
    ...
    const uint32_t write_index = header->write_index.load(std::memory_order_acquire);
    for (uint32_t read_index = 0; read_index != write_index;
         read_index = (read_index + 1) % static_cast<uint32_t>(kCapacity)) {
        decode_log_entry(buffer[read_index], header, buf, sizeof(buf), module_mapper_);
        out << buf;
        ++count;
    }

    if (header->version == 3 && header->buffer_var_size > 0 &&
        writer.size() >=
            sizeof(LogShmHeader) + kCapacity * sizeof(LogEntry) + static_cast<std::size_t>(header->buffer_var_size)) {
        const auto* buf_var = reinterpret_cast<const uint8_t*>(base + kCapacity * sizeof(LogEntry));
        const uint32_t write_index_var = header->write_index_var.load(std::memory_order_acquire);
        uint32_t pos = 0;

        while (pos < write_index_var) {
            const auto* hdr = reinterpret_cast<const LogVarEntryHeader*>(buf_var + pos);
            const uint8_t* payload = buf_var + pos + sizeof(LogVarEntryHeader);
            ...
            decode_log_entry_var(*hdr, payload, header, buf, sizeof(buf), module_mapper_);
            out << buf;
            ++count;

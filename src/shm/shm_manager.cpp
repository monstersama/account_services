...
shm_manager::shm_manager(shm_manager &&other) noexcept
    : name_(std::move(other.name_)), ptr_(other.ptr_), size_(other.size_),
      fd_(other.fd_) {
  other.ptr_ = nullptr;
  other.size_ = 0;
  other.fd_ = -1;
...
// 内部实现：打开或创建共享内存
void *shm_manager::open_impl(std::string_view name, std::size_t size,
                             shm_mode mode) {
  // 如果已经打开了其他共享内存，先关闭
  if (is_open()) {
    close();
  ...
  case shm_mode::Create:
    ...
    if (fd_ < 0) {
      std::cerr << "shm_open failed for " << name << ": " << strerror(errno)
                << std::endl;
      return nullptr;
    }
    is_new = true;
  ...
  struct stat st;
  if (fstat(fd_, &st) < 0) {
    std::cerr << "fstat failed for " << name << ": " << strerror(errno)
              << std::endl;
    cleanup(is_new);
    return nullptr;
  }
  ...
  if (!is_new && static_cast<std::size_t>(st.st_size) != size) {
    std::cerr << "shm size mismatch for " << name << ": expected " << size
              << ", got " << static_cast<std::size_t>(st.st_size) << std::endl;
    cleanup(false);
    return nullptr;
  }
  ...
  if (is_new) {
    if (ftruncate(fd_, static_cast<off_t>(size)) < 0) {
      std::cerr << "ftruncate failed for " << name << ": " << strerror(errno)
                << std::endl;
      cleanup(true);
      return nullptr;
    }
  ...
  if (ptr_ == MAP_FAILED) {
    std::cerr << "mmap failed for " << name << ": " << strerror(errno)
              << std::endl;
    cleanup(is_new);
    return nullptr;
  }
...
bool shm_manager::validate_header(const shm_header *header) {
  if (header->magic != shm_header::kMagic) {
    std::cerr << "Invalid shm magic: expected 0x" << std::hex
              << shm_header::kMagic << ", got 0x" << header->magic << std::dec
              << std::endl;
    return false;
  }
  if (header->version != shm_header::kVersion) {
    std::cerr << "Invalid shm version: expected " << shm_header::kVersion
...
// 创建/打开上游共享内存
upstream_shm_layout *shm_manager::open_upstream(std::string_view name,
                                                shm_mode mode,
...
// 创建/打开下游共享内存
downstream_shm_layout *shm_manager::open_downstream(std::string_view name,
                                                    shm_mode mode,
                                                    account_id_t account_id) {
...
// 创建/打开成交回报共享内存
trades_shm_layout *shm_manager::open_trades(std::string_view name,
                                            shm_mode mode,
                                            account_id_t account_id) {
...
// 创建/打开持仓共享内存
positions_shm_layout *shm_manager::open_positions(std::string_view name,
                                                  shm_mode mode,
                                                  account_id_t account_id) {
  ...
  if (last_open_is_new_) {
    ...
    if (layout->header.magic != positions_header::kMagic) {
      std::cerr << "Invalid positions shm magic: expected 0x" << std::hex
                << positions_header::kMagic << ", got 0x"
                << layout->header.magic << std::dec << std::endl;
      close();
      return nullptr;
    }
    if (layout->header.version != positions_header::kVersion) {
      std::cerr << "Invalid positions shm version: expected "
                << positions_header::kVersion << ", got "
...
bool shm_manager::unlink(std::string_view name) {
  if (shm_unlink(std::string(name).c_str()) < 0) {
    std::cerr << "shm_unlink failed for " << name << ": " << strerror(errno)
              << std::endl;
    return false;
  }
  return true;
...
// 检查是否已打开
bool shm_manager::is_open() const noexcept {

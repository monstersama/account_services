...
struct alignas(64) order_request {
  ...
  std::atomic<order_status_t> order_status{
      order_status_t::NotSet}; // 订单当前状态
  ...
  order_request(const order_request &other) {
    ...
    order_status.store(other.order_status.load(std::memory_order_relaxed),
  ...
  void init_new(std::string_view sec_id, internal_security_id_t internal_sec_id,
                internal_order_id_t internal_id, trade_side_t side,
                market_t mkt, volume_t vol, dprice_t dpx,
                md_time_t md_time_driven_) {
  ...
  void init_cancel(internal_order_id_t internal_id, md_time_t md_time_driven_,
                   internal_order_id_t orig_internal_id) {
    internal_order_id = internal_id;
...
static_assert(sizeof(order_request) == 192,

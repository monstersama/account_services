...
# 演示脚本：提交一笔 SH 600000 卖出 TWAP 订单。
...
if [[ ! -x "${SUBMIT_BIN}" ]]; then
  echo "[demo_submit_twap_sell] missing binary: ${SUBMIT_BIN}" >&2
  echo "[demo_submit_twap_sell] build the project first, for example:" >&2
...
echo "[demo_submit_twap_sell] running: ${cmd[*]}"
"${cmd[@]}"
